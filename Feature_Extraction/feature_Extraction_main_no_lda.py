    ...
    for subject in tqdm(os.listdir(data_dir)):
        ...
        for file in os.listdir(subj_path):
            ...
            if label_str not in label_map:
                print(f"⚠️ Skipping unknown label in file: {file}")
                continue
...
if __name__ == '__main__':
    ...
    X, y = load_data_and_extract_features(DATA_DIR, apply_pca=False, remove_rest=True)

    np.save('X_full_nolda.npy', X)
    np.save('y_full_nolda.npy', y)
